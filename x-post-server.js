...
const PORT = Number(process.env.X_POST_SERVER_PORT || 8787);
...
function safeRedirectUri(raw) {
  const fallback = `http://${HOST}:${PORT}/x-callback`;
...
async function handleCheckWordPress(req, res) {
  ...
  let user;
  try {
    user = await wordpressJson(payload.siteUrl, payload.username, payload.appPassword, "/users/me", {
      params: { context: "edit" }
    });
  } catch (error) {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && requestUrl.pathname === "/health") {
    sendJson(res, 200, { ok: true, service: "x-post-server", port: PORT });
    return;
  }
...
server.listen(PORT, HOST, () => {
  log(`X post server running at http://${HOST}:${PORT}`);
